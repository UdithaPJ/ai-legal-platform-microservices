    ...
    (async () => {
      try {
        ...
        const [clientData, appointmentData] = await Promise.all([
          apiFetch<UserResponse>(`/users/${current.clientId}`).catch(() => null),
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] flex-col overflow-hidden rounded-xl border bg-white">
      ...
      <div className="flex items-center gap-3 border-b px-4 py-3">
        ...
        <div className="flex size-9 items-center justify-center rounded-full bg-gray-100 text-sm font-bold text-gray-600">
          {getInitials(client?.fullName ?? "Client")}
        </div>
        <div className="flex-1">
          <p className="text-sm font-semibold text-gray-900">{client?.fullName ?? "Client"}</p>
